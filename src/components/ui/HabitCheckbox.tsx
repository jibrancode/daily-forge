...
export const HabitCheckbox: React.FC<HabitCheckboxProps> = ({
  ...
  return (
    <button
      ...
      className={`focus-ring flex min-h-12 w-full items-center justify-between gap-3 rounded-[var(--radius-md)] border p-3 text-left transition-all duration-[150ms] ${
        habit.completed
    ...
    >
      <span className={`min-w-0 text-sm font-medium leading-snug ${habit.completed ? 'line-through text-[var(--text-muted)]' : ''}`}>
        {habit.name}
      </span>

      <span
        className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-[var(--radius-sm)] border transition-colors duration-[150ms] ${
          habit.completed
